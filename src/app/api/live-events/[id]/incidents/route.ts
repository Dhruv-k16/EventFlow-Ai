...
export const GET = withAuth(async (req: NextRequest, ctx) => {
  ...
  try {
    ...
    const incidents = await prisma.incident.findMany({
      where: { liveEventId: id, ...(severity ? { severity } : {}), ...(openOnly ? { resolvedAt: null } : {}) },
      orderBy: [{ severity: 'desc' }, { createdAt: 'desc' }],
    }) as IncidentRow[];

    return NextResponse.json(incidents.map(i => ({ ...i, createdAt: i.createdAt.toISOString(), updatedAt: i.updatedAt.toISOString(), resolvedAt: i.resolvedAt?.toISOString() ?? null })));
  } catch (err) {
...
export const POST = withAuth(async (req: NextRequest, ctx) => {
  ...
  try {
    ...
    if (!VALID_SEVERITIES.includes(sev)) return NextResponse.json({ error: `severity must be one of: ${VALID_SEVERITIES.join(', ')}` }, { status: 400 });

    const incident = await prisma.incident.create({
      // FIX: Incident has no @default(uuid()) — must supply id + updatedAt
      data: {
